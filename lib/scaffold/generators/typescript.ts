/**
 * TypeScript/React project scaffold generator (Vite + React + TypeScript)
...
export class TypeScriptScaffoldGenerator implements ScaffoldGenerator {
  async generate(config: ScaffoldConfig): Promise<ScaffoldResult> {
    ...
    files.push({
      path: 'package.json',
      content: this.generatePackageJson(config.projectName),
    })
    ...
    files.push({
      path: 'index.html',
      content: this.generateIndexHtml(config.projectName),
    })
    ...
    return {
      files,
      message: `Successfully scaffolded TypeScript/React project: ${config.projectName}`,
    }
  }
...
This project was scaffolded as part of a Code Lab learning experience using Vite + React + TypeScript.
`
...
`
  ...
  private generateViteSvg(): string {
  ...
  private generateReactSvg(): string {
...
This command will:
1. Execute vitest with appropriate filters
